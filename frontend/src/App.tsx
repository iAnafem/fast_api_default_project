...
function App() {
  ...
  return (
    <div>
      ...
      <button type="button" onClick={() => handleClick(backend_url, setBackMsg)}>Say hello to backend</button>
      <p>{backend_msg}</p>
      <button type="button" onClick={() => handleClick(db_url, setDbMsg)}>Say hello to DB</button>
      <p>{db_msg}</p>
    </div>
  )
}
